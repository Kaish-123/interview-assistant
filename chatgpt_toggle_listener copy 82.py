...
class ChatGPTAssistant:
    def __init__(self):
        ...
        self.transcription_abort_event = threading.Event()
    
    def cancel_transcription(self):
        if self.transcription_thread and self.transcription_thread.is_alive():
            self.transcription_abort_event.set()
            self.transcription_thread.join(timeout=1)
            print("🛑 Transcription thread aborted.")
        self.transcription_abort_event.clear()
...
class Application(tk.Tk):
    ...
    def on_tab_select(self, event):
        ...
        if item_id.startswith("tab_"):
            ...
            # Insert prompt and text input into input field when subtab is selected
            ...
            if text_input:
                # Directly set text input in the entry field (this will be submitted automatically)
                self.input_entry.delete(0, tk.END)
                self.input_entry.insert(0, text_input)

                # Call the submit method directly to send the message
                self.submit_text_question()  # Automatically submit the text input when selected
    ...
    def toggle_recording(self):
        with self.toggle_lock:  # Ensure only one action is triggered at a time
            if self.is_processing_audio:
                print("⏳ Already processing. Cancelling previous input...")
                self.assistant.cancel_transcription()
                self.is_processing_audio = False
                self.toggle_recording()  # 🔁 Restart recording immediately
                return
            ...
            if not self.assistant.recorder.is_recording:
                ...
                self.assistant.transcription_thread = threading.Thread(
    target=self.process_recording, daemon=True
)
                self.assistant.transcription_thread.start()
    ...
    def process_recording(self):
        try:
            ...
            if self.assistant.transcription_abort_event.is_set():
                self.status.config(text="❌ Transcription cancelled.")
                return
