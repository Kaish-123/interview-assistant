    ...
    def stream_gpt_response(self, text_widget, status_label, button):
        ...
        def run_stream():
            with self.lock:
                ...
                try:
                    context_window = 4  # Only last 10 messages (adjust if needed)
                    #context_window = 10
                    if len(self.messages) > context_window:
                        messages_to_send = [self.messages[0]] + self.messages[-(context_window-1):]
                    else:
