...
class LiveTranscriber:
    """
    Periodically batches the newest audio frames into a short wav,
    sends to Whisper, and updates the UI with incremental text.
    """
    def __init__(self, recorder: AudioRecorder, ui_append_fn, client, sample_rate=16000, channels=1):
        self.recorder = recorder
        self.ui_append_fn = ui_append_fn  # function(text:str) -> None (must be thread-safe via .after)
        self.client = client
        ...
        self._thread = None
        self._running = False
        self._buffer = deque(maxlen=int(sample_rate * 12 / 1024) + 10)  # ~12s rolling window of chunks
        self._last_text = ""
        self._last_emit = 0.0
        self.interval_sec = 1.2          # how often to hit Whisper
        self.window_sec = 8.0            # how much audio to send each time (~8s improves accuracy)
        self.min_chars_per_emit = 12     # throttle UI spam
    ...
    def _collect_window_audio(self):
        # copy frames safely
        frames = list(self.recorder.frames[-int(self.sample_rate*self.window_sec/self.recorder.stream.blocksize)-5:])
        if not frames:
            return None
    ...
    def _loop(self):
        ...
        while self._running:
            now = time.time()
            if now - last_call < self.interval_sec:
                time.sleep(0.05)
                continue
            last_call = now

            audio = self._collect_window_audio()
            ...
            try:
                ...
                # Transcribe this short segment
                with open(path, "rb") as f:
                    resp = self.client.audio.transcriptions.create(
                        model="whisper-1",
                        file=f,
                        # You can hint punctuation/formatting:
                        # prompt="This is a technical interview. Use code-friendly punctuation."
                    )
                os.unlink(path)
                ...
                # Only append *new* delta compared to the last text we showed
                delta = text
                if text.startswith(self._last_text):
                    delta = text[len(self._last_text):]

                # Heuristic cleanup: avoid flicker on small backtracks
                if len(delta.strip()) >= self.min_chars_per_emit:
                    self._last_text = text
                    self.ui_append_fn(delta)

            except Exception as e:
                # Non-fatal: keep the loop alive
                # Comment the below print if it gets noisy
                print(f"[LiveTranscriber] warn: {e}")
                time.sleep(0.2)
...
class Application(tk.Tk):
    ...
    def _append_live_text(self, text_part: str):
    # Called from background thread via .after
        def do():
            self.live_box.config(state=tk.NORMAL)
            # Append delta; keep it to ~4 lines by trimming older content
            current = self.live_box.get("1.0", tk.END)
            if len(current) > 2000:
                self.live_box.delete("1.0", "3.0")  # trim a couple of lines
            self.live_box.insert(tk.END, text_part)
            self.live_box.see(tk.END)
            self.live_box.config(state=tk.DISABLED)
        self.after(0, do)
    ...
    def setup_ui(self):
        ...
        # Live transcript area (single-line feel, but allow wrapping)
        live_frame = ttk.Frame(self.main_frame)
        live_frame.pack(fill="x", padx=10, pady=(0, 6))
        ttk.Label(live_frame, text="🎧 Live transcript:", style='TLabel').pack(anchor="w")

        self.live_box = tk.Text(
            live_frame, height=3, wrap=tk.WORD, font=('Consolas', 12),
            bg='#2a2b32', fg='white', insertbackground='white', highlightthickness=0
        )
        self.live_box.pack(fill="x", expand=False)
        self.live_box.config(state=tk.DISABLED)
    ...
    def toggle_recording(self):
        with self.toggle_lock:
            ...
            if not self.assistant.recorder.is_recording:
                ...
                if self.assistant.live_transcriber:
                    try:
                        self.assistant.live_transcriber.stop()
                    except:
                        pass
                self.assistant.live_transcriber = LiveTranscriber(
                    recorder=self.assistant.recorder,
                    ui_append_fn=lambda t: self._append_live_text(t),
                    client=client,
                    sample_rate=SAMPLE_RATE,
                    channels=CHANNELS
                )
                self.assistant.live_transcriber.start()
                ...
                # Stop live updater before final transcription
                if self.assistant.live_transcriber:
                    self.assistant.live_transcriber.stop()
                    self.assistant.live_transcriber = None
