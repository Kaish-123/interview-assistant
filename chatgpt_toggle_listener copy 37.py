...
class ChatGPTAssistant:
    def __init__(self):
        ...
        self._streaming_insert_index = None
        self.messages = self.load_chat_history() or [{"role": "system", "content": "You are a helpful interview assistant. Provide detailed technical answers and ask follow-up questions when appropriate."}]
    ...
    def update_text_widget(self, text_widget):
        ...
        if self.streaming:
            if self._streaming_insert_index is None:
                text_widget.insert(tk.END, "Answer: ")
                self._streaming_insert_index = text_widget.index(tk.END)

            existing = text_widget.get(self._streaming_insert_index, tk.END)
            new_text = self.current_response[len(existing):]
            if new_text:
                text_widget.insert(tk.END, new_text)

        else:
            # ✅ Do NOT reprint assistant message again — it's already printed
            self._streaming_insert_index = None
...
def restore_chat_history_on_launch(assistant, response_box):
        if not os.path.exists(CHAT_HISTORY_FILE):
            return
        ...
        try:
            ...
            for msg in messages:
                ...
                if isinstance(msg["content"], list):
                    ...
                    response_box.insert("end", f"{prefix}{display_text.strip()}\n\n")
                else:
                    response_box.insert("end", f"{prefix}{msg['content']}\n\n")
...
class Application(tk.Tk):
    def __init__(self):
        ...
        self.setup_ui()
        restore_chat_history_on_launch(self.assistant, self.response_box)
        ...
        def save_chat_history(self):
            try:
                filepath = os.path.join(CHAT_HISTORY_DIR, "chat_history.txt")
                with open(filepath, "w", encoding="utf-8") as f:
                    for msg in self.assistant.messages:
                        ...
                        if isinstance(msg['content'], list):
                            for part in msg['content']:
                                if part['type'] == 'text':
                                    f.write(prefix + part['text'] + "\n")
                                elif part['type'] == 'image_url':
                                    f.write(prefix + "[Image Attached]\n")
                        else:
                            f.write(prefix + msg['content'] + "\n")
            except Exception as e:
    ...
    def process_recording(self):
        try:
            ...
            # Display the question before GPT responds
            self.response_box.config(state=tk.NORMAL)
