...
class Application(tk.Tk):
    ...
    def submit_text_question(self):
        ...
        if self.is_processing_audio:
            print("🛑 Cancelling ongoing processing due to new chat input...")
            self.assistant.cancel_streaming()
            self.assistant.recorder.is_recording = False
            self.is_processing_audio = False
            self.status.config(text="📝 Previous process stopped. Starting new input...")
    ...
    def toggle_recording(self):
        with self.toggle_lock:
            if self.is_processing_audio:
                print("🛑 Cancelling ongoing audio and GPT processing...")
                self.assistant.cancel_streaming()  # Stop GPT
                if self.assistant.recorder.is_recording:
                    self.assistant.recorder.is_recording = False  # Stop recording loop
                self.is_processing_audio = False  # Reset processing state
                self.status.config(text="🔁 Previous input cancelled. Starting fresh...")
