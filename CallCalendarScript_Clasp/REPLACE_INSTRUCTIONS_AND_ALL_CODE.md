# Where to replace what — all code

Do this in your **Google Apps Script** project (script.google.com → open CallCalendarScript).

---

## 1. FILE: CalculateEarningsIL_Optimized.gs

**Where:** In the left Files list, click **New_EarningIncremental.gs** (or whichever file contains the function `calculateEarningsIL_Optimized`).  
If you have a file named **EarningILOpt.gs** that only has this function, use that.

**What to do:** Select **all** code in the editor (Ctrl+A / Cmd+A), delete it, then **paste the code below**. Save (Ctrl+S / Cmd+S).

---

### Code for CalculateEarningsIL_Optimized.gs (paste this entire block)
...
function calculateEarningsIL_Optimized() {
  ...
  const DEFAULT_PRICE = 12500;   // Default for events that don't match any keyword (12.5k)
...
## 2. FILE: Wrapper (syncCalendarAndCalculateEarnings + CALSYNC / syncAndPrice_Rolling)

**Where:** In the left Files list, click **Wrapper_Script.gs.gs** (or the file that contains `syncCalendarAndCalculateEarnings` and `syncAndPrice_Rolling` / CALSYNC).  
If the wrapper and CALSYNC are in **different files**, replace the file that has the CALSYNC block and the two global functions at the bottom; keep `syncCalendarAndCalculateEarnings` in the same file (it’s at the top of the code below).

**What to do:** Select **all** code in that file, delete it, then **paste the code below**. Save (Ctrl+S / Cmd+S).

---

### Code for Wrapper file (paste this entire block)
...
| Step | Where (in Apps Script) | What to do |
|------|------------------------|------------|
| 1 | File that has **calculateEarningsIL_Optimized** (e.g. **New_EarningIncremental.gs** or **EarningILOpt.gs**) | Select all → delete → paste **Code for CalculateEarningsIL_Optimized.gs** above → Save |
| 2 | File that has **syncCalendarAndCalculateEarnings** and **syncAndPrice_Rolling** (e.g. **Wrapper_Script.gs.gs**) | Select all → delete → paste **Code for Wrapper file** above → Save |

Do **not** rename the functions (syncCalendarAndCalculateEarnings, calculateEarningsIL_Optimized, syncAndPrice_Rolling) or your triggers will break.  
After replacing, run **calculateEarningsIL_Optimized** once from the Run menu to test.
