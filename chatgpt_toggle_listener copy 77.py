...
class PromptManager:
    ...
    def add_subtab(self, tab_index, name, prompt="", text_input=""):
        # Adds a new subtab, including text_input, and saves it
        if 0 <= tab_index < len(self.data["tabs"]):
            self.data["tabs"][tab_index]["subTabs"].append({
                "name": name, 
                "prompt": prompt, 
                "text_input": text_input  # Make sure text_input is saved
            })
...
class PromptManager:
    ...
    def update_subtab_prompt(self, tab_index, subtab_index, prompt):
        # Updates the prompt and saves the data immediately
        if (0 <= tab_index < len(self.data["tabs"]) and 
            0 <= subtab_index < len(self.data["tabs"][tab_index]["subTabs"])):
            self.data["tabs"][tab_index]["subTabs"][subtab_index]["prompt"] = prompt
            self.save_tabs()  # Save data after modification
            return True
        return False
...
class Application(tk.Tk):
    ...
    def setup_ui(self):
        ...
        self.toggle_input_btn = ttk.Button(self.sidebar, text="🔈 Internal Audio (BlackHole)", command=self.toggle_input_mode)
        self.toggle_input_btn.pack(pady=5, fill="x")
        
        self.topmost_btn = ttk.Button(self.sidebar, text="📌 Pin", command=self.toggle_always_on_top)
