...
class Application(tk.Tk):
    ...
    def force_cancel(self):
        print("🔁 Forcing cancellation of previous tasks...")

        # Cancel GPT streaming
        self.assistant.cancel_streaming()

        # Stop audio recording if it's running
        if self.assistant.recorder.is_recording:
            try:
                self.assistant.recorder.is_recording = False
                if self.assistant.recorder.stream:
                    self.assistant.recorder.stream.stop()
                    self.assistant.recorder.stream.close()
            except Exception as e:
                print(f"⚠️ Error stopping stream: {e}")

        # Reset UI and flags
        self.is_processing_audio = False
        self.record_btn.config(text="🎤 Listen", state=tk.NORMAL)
        self.stop_btn.config(state=tk.DISABLED)
        self.status.config(text="⏹ Cancelled previous task")
    
    def is_recent_trigger(self, threshold=0.5):
        now = time.time()
        if now - self.last_input_time < threshold:
            return True
...
if __name__ == "__main__":
    ...
    def setup_hotkey_listener():
        ...
        def on_activate_listen_external():
            if app.is_recent_trigger():
                print("⚠️ Ignored rapid hotkey press.")
                return
