...
import queue
import ctypes
import PyPDF2  # for reading PDF resume/JD
        ...
        def stream_task():
            with self.lock:
                ...
                try:
                    ...
                    self.chat_history.append((prompt, self.current_response))
...
class Application(tk.Tk):
    ...
    def setup_ui(self):
        ...
        attach_btn_frame = ttk.Frame(self)
        attach_btn_frame.pack(pady=5)

        ttk.Button(attach_btn_frame, text="📎 Attach Resume", command=self.attach_resume).pack(side="left", padx=5)
        ttk.Button(attach_btn_frame, text="📄 Attach JD", command=self.attach_jd).pack(side="left", padx=5)
        ...
        for btn in [self.record_btn, self.stop_btn, self.submit_btn, self.topmost_btn]:
            btn.bind("<Enter>", lambda e: e.widget.config(cursor="hand2"))
            btn.bind("<Leave>", lambda e: e.widget.config(cursor="arrow"))
    ...
    def stop_output(self):
        ...
        self.stop_btn.config(state=tk.DISABLED)
        self.record_btn.config(state=tk.NORMAL, text="🎤 Start Listening")
        self.status.config(text="⏹ Output stopped and ready.")
    ...
    def attach_resume(self):
        ...
        if file_path:
            self.assistant.resume_content = self.read_file(file_path)
            self.status.config(text="📎 Resume attached")

    def attach_jd(self):
        file_path = filedialog.askopenfilename(filetypes=[("PDF or TXT", "*.pdf *.txt")])
        if file_path:
            self.assistant.jd_content = self.read_file(file_path)
            self.status.config(text="📄 JD attached")
