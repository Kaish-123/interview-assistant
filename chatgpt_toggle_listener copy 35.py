...
class ChatGPTAssistant:
    ...
    def update_text_widget(self, text_widget):
        ...
        # Find the index where last assistant message ends
        last_idx = text_widget.index(tk.END)

        # Only append the new delta chunk instead of redrawing the whole thing
        if self.streaming:
            if not hasattr(self, "_streaming_insert_index"):
                self._streaming_insert_index = last_idx
                text_widget.insert(tk.END, "Answer: ")
            text_widget.insert(tk.END, self.current_response[len(text_widget.get(self._streaming_insert_index, tk.END)) - 1:])
        else:
