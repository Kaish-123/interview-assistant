#!/bin/bash
# Self-healing launcher for cron_growth.py
...
if [ -z "$PYTHON" ]; then
    echo "[run_growth.sh] ERROR: No working Python found" >&2
    exit 1
fi

echo "[run_growth.sh] Using Python: $PYTHON"
...
if [ -d "$SITE_PKGS" ]; then
    ...
    echo "[run_growth.sh] Using site-packages: $SITE_PKGS"
fi

exec "$PYTHON" "$DIR/cron_growth.py" "$@"
